...
class product_product(models.Model):
    ...
    def get_add_to_cart_buttons(self):
        ...
        if self.purchase_type == 'none':
            ...
            res['list_view'] = u"""""" # There is never a reseller button on the list view. 
            res['product_view'] = u"""<button type="button" on('click', function(){getLocation(function(){alert('nu klickade du ja!')} ) } ) class="add_to_cart_consumer dn_btn dn_primary mt8 text-center {buy_button_hidden}" data-toggle="modal" data-target="#reseller_search" ">{text}</button>""".format(
                buy_button_hidden = '',
