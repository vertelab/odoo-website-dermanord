    ...
    def reseller(self, partner=None, country=None, city='', competence=None, **post):
        ...
        if not partner:
            ...
            if words and words != '':
                ...
                if len(post_codes) > 0:
                    for p in post_codes:
                        partner_ids += request.env['res.partner'].sudo().geo_zip_search('position', 'SE', '%s %s' %(p[:3], p[3:]), [('is_company', '=', True), ('is_reseller', '=', True), ('child_ids.type', '=', 'visit')], distance=0.5, limit=20)
                if len(strings) > 0:
                    for s in strings:
